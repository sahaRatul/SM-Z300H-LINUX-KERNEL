...
#endif
#ifdef CONFIG_SND_SOC_SPRD_USE_INTERNAL_DETECT
static int sprd_hp_detect_pre(struct snd_soc_codec *codec);
#endif
...
#ifdef CONFIG_SND_SOC_SPRD_USE_INTERNAL_DETECT
/*
open however 0x40038600 [4] = 1, 0x4003866c [14] = 1;
ear detect
*/
static int sprd_hp_detect_pre(struct snd_soc_codec *codec)
{
	...
	struct regulator *regulator_reg	= 0;

	int classg_vol	= 2200000;
	int is_reg_enabled = 0;
	int is_get_regulator_reg = 0;
	sp_asoc_pr_dbg("%s set vddclsg on\n", __func__);
	/*1.CG_EN */
	/* open clk */
	regulator_reg = regulator_get(0, "VREG");
	if (IS_ERR(regulator_reg)) {
		is_get_regulator_reg = 0;
		pr_err("ERR:Failed to request %ld: %s\n",
			   PTR_ERR(regulator_reg), "VREG");
		BUG_ON(1);
		return -1;
	} else {
		is_get_regulator_reg = 1;
	}
	regulator_set_mode(regulator_reg, REGULATOR_MODE_STANDBY);
	if (regulator_enable(regulator_reg) >= 0) {
		is_reg_enabled = 1;
		...
		is_reg_enabled = 0;
		pr_err("%s ERR:Failed to regulator_enable VREG\n", __func__);
	}
	if (1 == is_get_regulator_reg) {
		/* close clk */
		regulator_set_mode(regulator_reg, REGULATOR_MODE_NORMAL);
		if (1 == is_reg_enabled) {
			regulator_disable(regulator_reg);
			is_reg_enabled = 0;
		}

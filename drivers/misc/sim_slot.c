...
static int check_simslot_count(struct seq_file *m, void *v)
{
...
#ifdef CONFIG_MACH_Z3
	int support_number_of_simslot;
	support_number_of_simslot = DUAL_SIM;
	pr_info("Z3: SIM_SLOT_COUNT = %d\n", support_number_of_simslot);
#else
	...
	if (retval) {
		...
		if (retval) {
			...
			/* This codes are implemented assumption that count
			of GPIO about simslot is only one on H/W schematic.
			You may change this codes if count of GPIO about
			simslot has change */
			switch (retval) {
